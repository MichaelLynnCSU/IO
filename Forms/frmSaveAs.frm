...
Private Sub cmdSaveAs_Click()
    If Len(TextBox1) > 0 Then
        ActiveWorkbook.saveAs Filename:=Left(Sheets("File Paths").Cells(2, 2), InStrRev(Sheets("File Paths").Cells(2, 2), "\")) & TextBox1.Value & "_" & TextBox2.Value & "_IO Export_" & Format(Date, "MM-DD-YYYY") & ".xlsx", FileFormat:=xlOpenXMLWorkbook
        Application.DisplayAlerts = False
    Else:
        ActiveWorkbook.saveAs Filename:=Left(Sheets("File Paths").Cells(2, 2), InStrRev(Sheets("File Paths").Cells(2, 2), "\")) & ComboBox1.Value & "_" & TextBox2.Value & "_IO Export_" & Format(Date, "MM-DD-YYYY") & ".xlsx", FileFormat:=xlOpenXMLWorkbook
        Application.DisplayAlerts = False
    End If
